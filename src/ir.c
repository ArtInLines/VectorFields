...
bool isOp(char c)
{
	...
	STATIC_ASSERT(IR_META_INST_LAST_LASSOC - IR_META_INST_FIRST_LASSOC - 1 == 4);
	STATIC_ASSERT(IR_META_INST_LAST_RASSOC - IR_META_INST_FIRST_RASSOC - 1 == 1);
	return c == '+' || c == '-' || c == '*' || c == '/';
}
...
{
	STATIC_ASSERT(IR_META_INST_LEN == 33);
	STATIC_ASSERT(IR_META_INST_LAST_CHILDLESS < IR_META_INST_LAST_TRIG);
	STATIC_ASSERT(IR_META_INST_LAST_TRIG < IR_META_INST_LAST_UNARY);
	STATIC_ASSERT(IR_META_INST_LAST_UNARY < IR_META_INST_LAST_BINARY)
	STATIC_ASSERT(IR_META_INST_LAST_BINARY < IR_META_INST_LAST_TERTIARY);
	STATIC_ASSERT(IR_META_INST_LAST_TERTIARY < IR_META_INST_LAST_LASSOC);
	STATIC_ASSERT(IR_META_INST_LAST_LASSOC < IR_META_INST_LAST_RASSOC);
...
bool checkUserFunc(IR *root)
{
	...
	for (i32 inst = 0; inst < len; inst++) if (!checkUserFunc(&root->children[inst])) return false;
	if (inst == IR_INST_ROOT) return root->children[len-1].type == IR_TYPE_VEC2;
...
IR_Eval_Res evalUserFunc(IR node, Vector2 in)
{
	...
	switch (node.inst) {
		...
		case IR_INST_CONV: {
			if (node.type != IR_TYPE_FLOAT) TODO();
		...
		case IR_INST_MAX: {
			if (arrlen(node.children) != 2) return (IR_Eval_Res){0};
			IR_Eval_Res a = evalUserFunc(node.children[0], in);
			IR_Eval_Res b = evalUserFunc(node.children[1], in);
			if (!a.succ || !b.succ) return (IR_Eval_Res){ .val = {0}, .succ = false };
			IR_Val v = {0};
			switch (node.type) {
				case IR_TYPE_INT:   v.i = MAX(a.val.i, b.val.i); break;
				case IR_TYPE_FLOAT: v.f = MAX(a.val.f, b.val.f); break;
				case IR_TYPE_VEC2:  TODO();
				case IR_TYPE_ANY:
				case IR_TYPE_LEN:   UNREACHABLE();
			}
		...
		case IR_INST_MIN: {
			...
			IR_Val v = {0};
			switch (node.type) {
				case IR_TYPE_INT:   v.i = MIN(a.val.i, b.val.i); break;
				case IR_TYPE_FLOAT: v.f = MIN(a.val.f, b.val.f); break;
				case IR_TYPE_VEC2:  TODO();
				case IR_TYPE_ANY:
				case IR_TYPE_LEN:   UNREACHABLE();
			}
		...
		case IR_INST_CLAMP: {
			if (arrlen(node.children) != 3) return (IR_Eval_Res){0};
			IR_Eval_Res a = evalUserFunc(node.children[0], in);
			IR_Eval_Res b = evalUserFunc(node.children[1], in);
			IR_Eval_Res c = evalUserFunc(node.children[2], in);
			if (!a.succ || !b.succ || !c.succ) return (IR_Eval_Res){ .val = {0}, .succ = false };
			IR_Val v = {0};
			switch (node.type) {
				case IR_TYPE_INT:   v.i = CLAMP(a.val.i, b.val.i, c.val.i); break;
				case IR_TYPE_FLOAT: v.f = CLAMP(a.val.f, b.val.f, c.val.f); break;
				case IR_TYPE_VEC2:  TODO();
				case IR_TYPE_ANY:
				case IR_TYPE_LEN:   UNREACHABLE();
			}
		...
		case IR_INST_LERP: {
			...
			IR_Val v = {0};
			switch (node.type) {
				case IR_TYPE_INT:   v.i = LERP(a.val.i, b.val.i, c.val.i); break;
				case IR_TYPE_FLOAT: v.f = LERP(a.val.f, b.val.f, c.val.f); break;
				case IR_TYPE_VEC2:  TODO();
				case IR_TYPE_ANY:
				case IR_TYPE_LEN:   UNREACHABLE();
			}
